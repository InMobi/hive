...
create table table_desc3(key string, value string) clustered by (key)
sorted by (key DESC) into 1 BUCKETS;
...
insert overwrite table table_desc3 select key, value from src sort by key DESC;
...
explain
select /*+ mapjoin(b) */ count(*) from table_desc1 a join table_desc3 b
on a.key=b.key where a.key < 10;
