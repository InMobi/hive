...
public class TestBaseCubeQueries {
  ...
  public void testColumnErrors() throws Exception {
    ...
    th = null;
    try {
      rewrite(driver, "select msr11 + msr2 from basecube" +
          " where " + twoDaysRange);
    } catch (SemanticException e) {
      th = e;
      e.printStackTrace();
    ...
    th = null;
    try {
      rewrite(driver, "select dim1, cityid, msr3, msr13 from basecube" +
          " where " + twoDaysRange);
    } catch (SemanticException e) {
      th = e;
      e.printStackTrace();
  ...
  public void testMultipleFacts() throws Exception {
    String hqlQuery = rewrite(driver, "select dim1, roundedmsr2, msr12 from basecube" +
        " where " + twoDaysRange);
    String expected1 = getExpectedQuery(cubeName,
        "select basecube.dim1 dim1, sum(basecube.msr12) msr12 FROM ", null,
        " group by basecube.dim1",
        getWhereForDailyAndHourly2days(cubeName, "C1_testFact2_BASE"));
    ...
    hqlQuery = rewrite(driver, "select dim1, msr12, roundedmsr2 from basecube" +
        " where " + twoDaysRange);
    ...
    hqlQuery = rewrite(driver, "select dim1, msr12, roundedmsr2, msr13, msr3 from basecube" +
        " where " + twoDaysRange);
    expected1 = getExpectedQuery(cubeName,
        "select basecube.dim1 dim1, sum(basecube.msr12) msr12 FROM ", null,
        " group by basecube.dim1",
    ...
    expected2 = getExpectedQuery(cubeName,
        ...
        " group by basecube.dim1",
        getWhereForDailyAndHourly2days(cubeName, "C1_testFact1_BASE"));
    String expected3 = getExpectedQuery(cubeName,
    ...
    hqlQuery = rewrite(driver, "select dim1, dim11, msr12, roundedmsr2 from basecube" +
        " where " + twoDaysRange);
    expected1 = getExpectedQuery(cubeName,
        "select basecube.dim1 dim1, basecube.dim11 dim11, sum(basecube.msr12) msr12 FROM ", null,
        " group by basecube.dim1",
        getWhereForDailyAndHourly2days(cubeName, "C1_testFact2_BASE"));
    expected2 = getExpectedQuery(cubeName,
    ...
    hqlQuery = rewrite(driver, "select dim1, msr11, roundedmsr2 from basecube" +
        " where " + twoDaysRange);
    expected1 = getExpectedQuery(cubeName,
        "select basecube.dim1 dim1, basecube.msr11 msr11 FROM ", null,
        null,
    ...
    hqlQuery = rewrite(driver, "select dim1 d1, msr12 `my msr12`, roundedmsr2 m2 from basecube" +
        " where " + twoDaysRange);
    expected1 = getExpectedQuery(cubeName,
        "select basecube.dim1 d1, sum(basecube.msr12) expr2 FROM ", null,
        " group by basecube.dim1",
        getWhereForDailyAndHourly2days(cubeName, "C1_testFact2_BASE"));
    expected2 = getExpectedQuery(cubeName,
    ...
    hqlQuery = rewrite(driver, "select dim1, avg(msr12), avg(msr2) from basecube" +
        " where " + twoDaysRange);
    expected1 = getExpectedQuery(cubeName,
        "select basecube.dim1 dim1, avg(basecube.msr12) msr12 FROM ", null,
        " group by basecube.dim1",
        getWhereForHourly2days(cubeName, "C1_testfact2_raw_base"));
    expected2 = getExpectedQuery(cubeName,
    ...
    // query with join
    hqlQuery = rewrite(driver, "select testdim2.name, msr12, roundedmsr2 from basecube" +
        " where " + twoDaysRange);
    expected1 = getExpectedQuery(cubeName,
        "select testdim2.name name, sum(basecube.msr12) msr12 FROM ",
    ...
    hqlQuery = rewrite(driver, "select dim2, msr13, roundedmsr2 from basecube" +
        " where " + twoDaysRange);
    expected1 = getExpectedQuery(cubeName,
        "select testdim2.id dim2, max(basecube.msr13) msr13 FROM ", 
        " JOIN " + getDbName() + "c1_testdim2tbl testdim2 ON basecube.dim12 = " +
            " testdim2.id and (testdim2.dt = 'latest') ", null,
            " group by testdim2.id", null, 
    ...
    hqlQuery = rewrite(driver, "select booleancut, round(sum(msr2)/1000), avg(msr13 + msr14) from basecube" +
        " where " + twoDaysRange);
    expected1 = getExpectedQuery(cubeName,
